...
many different types of staircases can be built with each amount of bricks, 
...
with the first step having a height of 2 and the second step having a height of 1: 
(# indicates a brick)
...
def count(curr, left):
    global CACHE
    if CACHE[curr][left] != -1:
        return CACHE[curr][left]
    # current step-height & the stairs left
    if left == 0:
        return 1
    if curr > left:
        return 0
    result = count(curr + 1, left - curr) + count(curr + 1, left)
    CACHE[curr][left] = result
    return result


def solution(n):
    global CACHE
    CACHE = [[-1]*(n+1) for x in range(0, n + 2)]
    return count(1, n) - 1


print(solution(200))
# for x in range(1, 12):
#     print('x='+str(x)+' '+str(solution(x)))
