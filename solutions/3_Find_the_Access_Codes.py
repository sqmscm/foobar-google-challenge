...
number of passcodes changes daily. Commander Lambda gets a report every day that includes 
the locks' access codes, but only she knows how to figure out which of several lists contains 
the access codes. You need to find a way to determine which list contains the access codes once you're ready to go in. 

Fortunately, now that you're Commander Lambda's personal assistant, she's confided to you that she
 made all the access codes "lucky triples" in order to help her better find them in the lists. 
...
def solution(l):
    ...
    for x in range(0, len(l)):
        for y in range(x + 1, len(l)):
            if l[y] % l[x] == 0:
                cache[y] += 1  # record the counts
                # y % x = 0, so we add all nums % x = 0 to the tuple
                count += cache[x]
    return count


print(solution([1, 2, 6, 4, 5, 3]))
