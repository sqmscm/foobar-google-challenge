...
quickly create large numbers for use in the code, given a limited number of 
plates to work with.
...
and is divisible by 3. If it is not possible to make such a number, return 0 as the 
solution. L will contain anywhere from 1 to 9 digits. The same digit may appear 
...
def solution(l):
    ...
    for length in range(1, len(l) + 1):
        l_perm = list(permutations(l, length))
        for perm in l_perm:
            list_str = [str(x) for x in perm]
            nums.append(int(''.join(list_str)))
